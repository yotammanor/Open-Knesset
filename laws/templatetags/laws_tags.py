...
def selection(page_obj, paginator, request):
    ...
    if paginator.num_pages <= 10:
        show_pages = [[x,"?%s&page=%d"%(base_link,x),False] for x in paginator.object_list]
        show_pages[page_obj.number][2] = True
    else:
        if page_obj.number <= 5:
            show_pages = [[x,"?%s&page=%d"%(base_link,x),False] for x in paginator.object_list[:page_obj.number+2]]
            last_pages = [[x,"?%s&page=%d"%(base_link,x),False] for x in paginator.object_list[paginator.num_pages-2:paginator.num_pages]]
        elif page_obj.number >= paginator.num_pages-5:
            show_pages = [[x,"?%s&page=%d"%(base_link,x),False] for x in paginator.object_list[page_obj.number-4:paginator.num_pages]]
            first_pages = [[x,"?%s&page=%d"%(base_link,x),False] for x in paginator.object_list[:2]]
        else:
            first_pages = [[x,"?%s&page=%d"%(base_link,x),False] for x in pagintaor.object_list[:2]]
            last_pages = [[x,"?%s&page=%d"%(base_link,x),False] for x in paginator.object_list[paginator.num_pages-2:paginator.num_pages]]
            show_pages = [[x,"?%s&page=%d"%(base_link,x),False] for x in paginator.object_list[page_obj.number-3:page_obj.number+2]]
