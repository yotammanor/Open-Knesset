...
class PdfToolnameNotFoundException(Exception):
    pass
...
def pdftotext_version():
    if not PDFTOTEXT:
        return ('0', '0', '0')
    p = subprocess.Popen(executable=str(PDFTOTEXT), args=[str(PDFTOTEXT), '-v'], stderr=subprocess.PIPE,
                         stdout=subprocess.PIPE)
    major, minor, patchlevel = map(lambda x, y: y if x is None else x,
                                   p.stderr.readlines()[0].strip().split()[-1].split('.'), [0] * 3)
    p.kill()
...
if DEBUG:
    ...
    print "pdfinfo from %s" % str(PDFINFO)
...
PDF version:    1.4
    ...
    data = [(k, convert(k, v.strip())) for k, v in
            ((camel_to_lower_case(k), v.strip()) for k, v in
             (l.split(':', 1) for l in capture_output([str(PDFINFO), filename])))]
