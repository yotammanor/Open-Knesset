...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding field 'Committee.hide'
        db.add_column(u'committees_committee', 'hide',
                      self.gf('django.db.models.fields.BooleanField')(default=False),
                      keep_default=False)

        if not db.dry_run:
            orm.Committee.objects.filter(name__in=[
                u"הוועדה המיוחדת לדיון בהצעת חוק-יסוד: הממשלה (תיקון מס' 3 והוראת שעה לכנסת ה-20)",
                u"הוועדה המסדרת",
                u"הוועדה המשותפת לנושא סביבה ובריאות",
                u"וועדה מיוחדת לדיון בהצעת חוק-יסוד: הממשלה (תיקון מס' 3 והוראת שעה לכנסת ה-20)",
                u"ועדה מיוחדת לבעיית העובדים הזרים",
                u"ועדה מיוחדת לדיון בהצעת החוק בעניין השוויון בנטל בשירות הצבאי, בשירות האזרחי ובשוק העבודה ולהסדרת מעמדם של תלמידי הישיבות",
                u"ועדה מיוחדת לדיון בהצעת חוק השידור הציבורי",
                u"ועדה מיוחדת למאבק בנגע הסמים",
                u"ועדה מסדרת",
                u"ועדת חקירה פרלמנטרית בנושא קליטת עובדים ערבים בשירות הציבורי",
                u"ועדת משנה למאבק בסחר בנשים",
            ]).update(hide=True)

    def backwards(self, orm):
        # Deleting field 'Committee.hide'
        db.delete_column(u'committees_committee', 'hide')
